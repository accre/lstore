...
usage() {
    echo "Usage: $0 {start|stop|status|restart [-f]|cleanup [-f|-k] [older-than-date]|install <path> <user>}"
...
LIO_OPTS="-d 1"
LIO_CONFIG="-c lstore://10.0.13.241:cluster-lfs"
RUN_AS_USER=uscms01
#RUN_AS_USER=lfs
WDIR=/var/run/lfs
FORCE_UMOUNT="0"
...
start_instance_attempt() {
    ...
    copy_libs ${INSTANCE_PATH}
    copy_binary ${INSTANCE_PATH}
...
# ^\ Quit (core dumped)
    ...
    if [ "${TCM}" != "" ]; then
        echo "Using TCMALLOC: ${TCM}"
        TCOPT="LD_PRELOAD=${TCM}"
    else
        echo "Missing TCMALLOC!"
        TCOPT=""
    fi
#    echo sudo -u ${RUN_AS_USER} bash -c "ulimit -c unlimited; ulimit -n 20480; TCMALLOC_RELEASE_RATE=5 LD_LIBRARY_PATH=${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}${INSTANCE_PATH:+:${INSTANCE_PATH}/lib} LD_PRELOAD=${INSTANCE_PATH}/lib/libtcmalloc.so ${INSTANCE_PATH}/${FUSE_BIN_REL_PATH} ${FUSE_OPTS} $INSTANCE_MNT --lio -C ${WDIR} -log ${INSTANCE_PATH}/logs/fuse.cfg $LIO_OPTS ${LIO_CONFIG}"
    set -o xtrace
    sudo -u ${RUN_AS_USER} timeout -s 9 30 bash -c "ulimit -c unlimited; ulimit -n 20480; TCMALLOC_RELEASE_RATE=5 LD_LIBRARY_PATH=${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}${INSTANCE_PATH:+:${INSTANCE_PATH}/lib} ${TCOPT} ${INSTANCE_PATH}/${FUSE_BIN_REL_PATH} ${FUSE_OPTS} $INSTANCE_MNT --lio -C ${WDIR} -log ${INSTANCE_PATH}/logs/fuse.cfg $LIO_OPTS ${LIO_CONFIG}"
    RESULT=$?
    set +o xtrace
