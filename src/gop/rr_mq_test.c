...
char *worker_host_string = "tcp://127.0.0.1:6715";
mq_msg_t *host = NULL;
mq_msg_t *worker_host = NULL;
...
void client_ping_test() {
	...
	msg = mq_msg_new();
	mq_msg_append_msg(msg, host, MQF_MSG_KEEP_DATA);
...
void worker_register_test() {
	...
	worker_portal = mq_portal_create(mqc, worker_host_string, MQ_CMODE_SERVER);
	...
	mq_msg_append_mem(msg, "WORKER", 6, MQF_MSG_KEEP_DATA);
	mq_msg_append_mem(msg, MQF_REGISTER_KEY, MQF_REGISTER_SIZE, MQF_MSG_KEEP_DATA);
	mq_msg_append_mem(msg, worker_host_string, strlen(worker_host_string), MQF_MSG_KEEP_DATA);
	mq_msg_append_mem(msg, &free_slots, 4, MQF_MSG_KEEP_DATA); // THIS 4 IS HARDCODED
	mq_msg_append_mem(msg, NULL, 0, MQF_MSG_KEEP_DATA);
...
int main(int argc, char **argv) {
	...
	apr_pool_create(&mpool, NULL);
        ...
        worker_host = mq_string_to_address(worker_host_string);
