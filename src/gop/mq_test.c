...
int unpack_msg(mq_msg_t *msg, test_data_t *td)
{
  ...
  //** Parse the response
  f = mq_msg_first(msg); mq_get_frame(f, (void **)&data, &n);
  if (n != 0) {  //** SHould be an empty frame
     log_printf(0, " ERROR:  Missing initial empty frame!\n");
     status = 1;
     goto fail;
  }
  f = mq_msg_next(msg); mq_get_frame(f, (void **)&data, &n);
  ...
  f = mq_msg_next(msg); mq_get_frame(f, (void **)&id, &n);
...
int bulk_test(mq_context_t *mqc)
{
  ...
  while ((gop = opque_waitany(q)) != NULL) {
     ...
     if (td->success_value != status.op_status) {
        ...
        log_printf(0, "TEST: (END) gid=%d bulk_ping_test(delay=%d, reply=%d, dt=%d) = FAIL (g=%d, s=%d) dt=%lf sid=%s\n", gop_id(gop), td->delay, td->address_reply, td->dt, status.op_status, td->success_value, ttime, mq_id2str((char *)&(td->id), sizeof(td->id), b64, sizeof(b64)));
     }

     dt = apr_time_now() - start_time;
     ttime = (1.0*dt) / APR_USEC_PER_SEC;
     log_printf(0, "BULK n=%d err=%d gid=%d dt=%lf sid=%s\n", n, err, gop_id(gop), ttime, mq_id2str((char *)&(td->id), sizeof(td->id), b64, sizeof(b64))); flush_log();
     dt = apr_time_now();
     gop_free(gop, OP_DESTROY);
     dt = apr_time_now() - dt;
     ttime = (1.0*dt) / APR_USEC_PER_SEC;
...
//}
  ...
  f = mq_msg_first(msg); mq_frame_destroy(mq_msg_pluck(msg, 0));  //blank
...
log_printf(1, "TEXEC sid=%s\n", mq_id2str(data, size, b64, sizeof(b64)));
  ...
  track_response = NULL;
log_printf(5, "address_reply=%d\n", td->address_reply);
  if (td->address_reply == 1) {  //** We're sending a TRACKADDRESSS reply so form the core
     ...
     mq_msg_append_mem(track_response, copy, size, MQF_MSG_AUTO_FREE);
  }

  //** what's left is the return address including a blank so just reverse it for the destination
  //** The tracking address needs to be tweaked by removing the source address line and adding
  //** my address.
  //** The variable "i"  is used to keep track of the source address line (i=1) which the tracking address
  //** needs to drop.
  i =0;
  response = mq_msg_new();
  while ((f = mq_msg_pop(msg)) != NULL) {
     mq_msg_push_frame(response, f);
     ...
     if (track_response != NULL) {  //** Got to add the routing info for the trackaddress message
        ...
        //** This is the sending address which goes at the beginning
        mq_get_frame(f, (void **)&data, &size);
        if (data != NULL) {
           type_malloc(copy, void, size);
           memcpy(copy, data, size);
        ...
        mq_msg_push_mem(track_response, copy, size, MQF_MSG_AUTO_FREE);
     }

     i++;
  }

  if (track_response != NULL) {  //** WE need to add ourselves to the tracking address
     mq_msg_append_mem(track_response, host, strlen(host), MQF_MSG_KEEP_DATA);
     mq_msg_append_mem(track_response, NULL, 0, MQF_MSG_KEEP_DATA);   //** and also append a NULL
  }

  //** Now form the rest of the messeage
  mq_msg_append_mem(response, MQF_VERSION_KEY, MQF_VERSION_SIZE, MQF_MSG_KEEP_DATA);
  mq_msg_append_mem(response, MQF_RESPONSE_KEY, MQF_RESPONSE_SIZE, MQF_MSG_KEEP_DATA);
  mq_msg_append_frame(response, pid);
  mq_msg_append_frame(response, tdf);
...
int server_handle_request(mq_socket_t *sock)
{
  ...
  f = mq_msg_first(msg); mq_get_frame(f, (void **)&data, &size);
  ...
  f = mq_msg_next(msg); mq_get_frame(f, (void **)&data, &size);
