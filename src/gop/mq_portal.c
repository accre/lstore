...
void mq_command_table_destroy(mq_command_table_t *t)
{
  ...
  for (hi=apr_hash_first(t->mpool, t->table); hi != NULL; hi = apr_hash_next(hi)) {
     apr_hash_this(hi, NULL, NULL, &val); cmd = (mq_command_t *)val;
...
log_printf(5, "id_size=%d\n", size);
  ...
  if (tn == NULL) {  //** Nothing matches so drop it
     log_printf(1, "ERROR: No matching ID! sid=%s\n", mq_id2str(id, size, b64, sizeof(b64))); flush_log();
     mq_msg_destroy(msg);
     return;
  }
...
log_printf(5, "tn->tracking=%p\n", tn->tracking);
     ...
     f = mq_msg_first(msg); mq_frame_destroy(mq_msg_pluck(msg, 0)); // empty
...
log_printf(5, "pending_count=%d npoll=%d conn_dead=%d do_conn_hb=%d n=%d dt=%lf\n", pending_count, npoll, conn_dead, do_conn_hb, n, dts);
  return(n+conn_dead);
}
...
int mq_conn_create(mq_portal_t *p, int dowait)
{
  ...
  //** Make the connection UUID which is p->name:conn_counter
//  snprintf(uuid, 1024, "%s:%d", p->host, p->counter);
//  p->counter++;
...
void mq_portal_destroy(mq_portal_t *p)
{
  ...
  log_printf(2, "host=%s closed_size=%d total_conn=%d\n", p->host, stack_size(p->closed_conn), p->total_conn); flush_log();
