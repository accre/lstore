...
void *client_test_thread(apr_thread_t *th, void *arg)
{
  ...
  gop = test_gop(mqc, 0, 0, 0, single_max, single_send, 10, 100); gop_waitall(gop); n += client_consume_result(gop);  //** Normal valid usage pattern
  gop = test_gop(mqc, 1, 0, 0, single_max, single_send, 10, 101); gop_waitall(gop); n += client_consume_result(gop);  //** Launch the flusher but no delay sending data
  gop = test_gop(mqc, 0, 0, 10, single_max, single_send, 5, 102); gop_waitall(gop); n += client_consume_result(gop);  //** Response will come after the timeout
  gop = test_gop(mqc, 1, 0, 15, single_max, single_send, 8, 103); gop_waitall(gop); n += client_consume_result(gop);  //** Launch the flusher but delay sending data forcing the heartbeat to handle it
  gop = test_gop(mqc, 0, 30, 0, single_max, single_send, 5, 104); gop_waitall(gop); n += client_consume_result(gop);  //** Vvalid use pattern but the client pauses after reading
