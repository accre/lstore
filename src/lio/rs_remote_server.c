...
void rsrs_abort_cb(void *arg, mq_task_t *task)
{
  ...
  f = mq_msg_first(msg); f = mq_msg_pop(msg); mq_get_frame(f, (void **)&data, &n);
  ...
  f = mq_msg_pop(msg); mq_get_frame(f, (void **)&data, &n);
  ...
  //** Get the ID frame
  fid = mq_msg_pop(msg); mq_get_frame(fid, (void **)&data, &n);
  if (n == 0) {
...
resource_service_fn_t *rs_remote_server_create(void *arg, inip_file_t *fd, char *section)
{
  ...
  assert(apr_pool_create(&(rsrs->mpool), NULL) == APR_SUCCESS);
  ...
  //** Get the MQC
  assert((rsrs->mqc = lookup_service(ess, ESS_RUNNING, ESS_MQ)) != NULL);
