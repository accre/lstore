...
int main(int argc, char **argv)
{
    ...
    assert_result(compare_buffers_print(buffer, base_data, bufsize, 0), 0);
    ...
    assert_result(gop_sync_exec(segment_copy(lio_gc->tpc_unlimited, lio_gc->da, NULL, seg, clone, 0, 0, bufsize, chunk_size, buffer, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(clone, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, base_data, bufsize, 0), 0);
    ...
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(seg, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, log1_data, bufsize, 0), 0);

    //*************************************************************************
    //------------------- Merge_with base and verify --------------------------
    //*************************************************************************
    assert_result(gop_sync_exec(slog_merge_with_base(seg, lio_gc->da, chunk_size, buffer, 1, lio_gc->timeout)), OP_STATE_SUCCESS);
    ...
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(clone, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, log2_data, bufsize, 0), 0);
    ...
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(clone2, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, log2_data, bufsize, 0), 0);
    ...
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(clone2, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, log3_data, bufsize, 0), 0);
    ...
    memset(buffer, 0, bufsize);
    assert_result(gop_sync_exec(segment_read(clone3, lio_gc->da, NULL, 1, &ex_iov, &tbuf, 0, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(compare_buffers_print(buffer, log3_data, bufsize, 0), 0);
    ...
    //** Clean up
    assert_result(gop_sync_exec(segment_remove(clone3, lio_gc->da, lio_gc->timeout)), OP_STATE_SUCCESS);
    assert_result(gop_sync_exec(segment_remove(clone2, lio_gc->da, lio_gc->timeout)), OP_STATE_SUCCESS);
