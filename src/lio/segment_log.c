  ...
  while ((ir = (slog_range_t *)next_interval_skiplist(&it)) != NULL) {
    ...
    if (ir->lo < r->lo) {  //** Need to truncate the 1st portion (and maybe the end)
       if (ir->hi > r->hi) {  //** Straddles r
          ...
          ir2->lo = r->hi+1;  ir2->hi = ir->hi;  ir2->data_offset = ir->data_offset + (ir2->lo - ir->lo);
      ...
      irlo = ir->lo;
      ir->lo = r->hi+1;  ir->data_offset = ir->data_offset + (ir->lo - irlo);
      insert_interval_skiplist(s->mapping, (skiplist_key_t *)&(ir->lo), (skiplist_key_t *)&(ir->hi), (skiplist_data_t *)ir);
    }
...
log_printf(15, "seg=" XIDT " n_iov=%d offset[0]=" XOT " len[0]=" XOT "\n", sw->seg, sw->n_iov, sw->iov[0].offset, sw->iov[0].len);
  ...
  table_offset = s->log_size;  s->log_size += sw->n_iov*sizeof(slog_range_t);
  data_offset = s->data_size;  s->data_size += nbytes;
...
op_status_t seglog_read_func(void *arg, int id)
{
  ...
  n_iov = 0;
  for (i=0; i< sw->n_iov; i++) {
     lo = iov[i].offset;  hi = lo + iov[i].len - 1;
...
op_status_t seglog_truncate_func(void *arg, int id)
{
  ...
  if (gop == NULL) { opque_free(q, OP_DESTROY); return(op_success_status); }
...
log_printf(15, "seglog_destroy: seg->id=" XIDT " ref_count=%d\n", segment_id(seg), seg->ref_count);
  ...
  if (s->table_seg != NULL) { atomic_dec(s->table_seg->ref_count); segment_destroy(s->table_seg); }
  if (s->data_seg != NULL) { atomic_dec(s->data_seg->ref_count); segment_destroy(s->data_seg); }
  if (s->base_seg != NULL) { atomic_dec(s->base_seg->ref_count); segment_destroy(s->base_seg); }
...
op_status_t seglog_merge_with_base_func(void *arg, int id)
{
  ...
  for (i=0; i<=n_iov; i++) {
     if (i < n_iov) {
        ...
        ex_in[i].offset = r->data_offset; len = r->hi - r->lo + 1;  ex_in[i].len = len;
        ex_out[i].offset = r->lo;  ex_out[i].len = len;
