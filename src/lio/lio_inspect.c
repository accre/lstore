...
log_printf(5, "AUTO RID=%s used=" XOT " total=" XOT "\n", re->ri.rid->rid_key, re->used, re->total);
      ...
      while ((re = get_ele_data(pe->rids)) != NULL) {
         ...
         d1 = re->ri.rid->delta; d2 = re->ri.rid->tolerance;
...
//  rid_inspect_tweak_t *ri;
  ...
  while ((pe = get_ele_data(pools)) != NULL) {
     ...
     total_ntodo += ntodo; total_ptodo += ptodo; total_todo += todo; total_finished += finished;
     tpos += ppos; tneg += pneg;
     move_down(pools);
  }
...
//printf("argc=%d\n", argc);
  ...
  acount = 1;
  key_rebalance = NULL;
  rtol = 0.05; rtol_mode = DELTA_MODE_PERCENT;
  ...
  do {
     ...
     if (strcmp(argv[i], "-rd") == 0) { //** Recurse depth
        ...
        rtol = string_get_double(value); i++;
