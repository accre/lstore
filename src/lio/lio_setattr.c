...
//printf("argc=%d\n", argc);
  if (argc < 2) {
     printf("\n");
     printf("lio_setattr LIO_COMMON_OPTIONS [-rd recurse_depth] [-t object_types] [-delim c] -p path -as key=value | -af key=vfilename \n");
     printf("lio_setattr LIO_COMMON_OPTIONS [-rd recurse_depth] [-t object_types] LIO_PATH_OPTIONS -as key=value | -af key=vfilename\n");
     ...
     printf("    -as key=value      - Breaks up the literal string into the key/value pair and stores it.  Multiple -as/-af clauses are allowed\n");
     printf("    -af key=vfilename  - Similar to -as but the valure is loaded from the given vfilename.  Multiple -as/-af clauses are allowed\n");
     return(1);
  }
