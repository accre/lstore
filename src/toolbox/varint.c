...
//     The number of bytes used are returned.  The buffer shoulbe be at least 16 bytes
//*******************************************************************************
...
int varint_decode(uint8_t *buffer, int bufsize, uint64_t *value)
{
    ...
    *value = 0;
    for (i=0, bits = 0; i<bufsize; i++, bits += 7) {
        *value += (uint64_t)(buffer[i] & 0x7F) << bits;
...
int tbx_varint_test()
{
    ...
    int asize = 12;
    uint64_t varray[] = {0, 1, 127, 128, 129, 16383, 16384, 16385, 2097151, 2097152, 2097153, 2348298882212802647};
    int      nbytes[] = {1, 1,   1,   2,   2,     2,     3,     3,       3,       4,       4,                   9};
