...
double write_allocs(ibp_capset_t *caps, int qlen, int n, int asize, int block_size)
{
  ...
  if (rem > 0) nblocks++;
  block_start = 0; alloc_start = 0;

  finished = 0;
  apr_poll(&pfd, nfds, &finished, dt);
  count = 0;
  nbytes=0;  last_bytes = 0;
...
//     ibp_set_min_depot_threads(ic, nthreads);
     ...
     q_len = atol(argv[i]); i++;
     n_allocs = atol(argv[i]); i++;
     alloc_size = atol(argv[i])*1024; i++;
     block_size = atol(argv[i])*1024; i++;
  }
  ...
  r1 = n_allocs; r1 = r1 * alloc_size / (1024.0*1024.0);
