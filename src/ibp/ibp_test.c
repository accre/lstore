...
printf("base_iovec_test: after allocate: nfinished=%d\n", opque_tasks_finished(q));
  ...
  n_ele = bcount; if (remainder > 0) n_ele++;
...
void perform_big_alloc_tests(ibp_depot_t *depot)
{
  ...
  //** Init the buffers
  buffer[bufsize] = '\0'; memset(buffer, '*', bufsize);
  buffer_cmp[bufsize] = '\0'; memset(buffer_cmp, '_', bufsize);
...
void perform_manage_truncate_tests(ibp_depot_t *depot)
{
  ...
  //** Init the buffers
  pos = bufsize/3;
  buffer[bufsize] = '\0'; memset(buffer, '*', bufsize); buffer[pos-1] = 'A'; buffer[pos] = 'R'; buffer[bufsize-1] = 'T';
  buffer_cmp[bufsize] = '\0'; memset(buffer_cmp, '_', bufsize);
...
//printf("Before allocate\n"); fflush(stdout);
  ...
  if (err != OP_STATE_SUCCESS) { printf("ibp_manage error = %d * ibp_errno=%d\n", err, IBP_errno); }
  err = IBP_manage(get_ibp_cap(caps, IBP_MANAGECAP), &timer, IBP_PROBE, 0, &astat);
  if (err == 0) {
...
//  set_ibp_attributes(&(astat.attrib), time(NULL) + 20, IBP_HARD, IBP_BYTEARRAY);
  ...
  if (err != 1024) { failed_tests++; printf("ibp_copy FAILED size = %d * ibp_errno=%d\n", err, IBP_errno); }
  ...
  if (err != 1024) { failed_tests++; printf("ibp_phoebus_copy FAILED size = %d * ibp_errno=%d\n", err, IBP_errno); }
  ...
  if (err != 0) { failed_tests++; printf("ibp_manage(decr) FAILED for caps1 error = %d * ibp_errno=%d\n", err, IBP_errno); }
  err = IBP_manage(get_ibp_cap(caps2, IBP_MANAGECAP), &timer, IBP_DECR, IBP_READCAP, &astat);
  if (err != 0) { failed_tests++; printf("ibp_manage(decr) FAILED for caps2 error = %d * ibp_errno=%d\n", err, IBP_errno); }
