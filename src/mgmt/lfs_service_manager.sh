...
# ^\ Quit (core dumped)
    ...
    echo sudo -u ${RUN_AS_USER} bash -c "ulimit -c unlimited; ulimit -n 20480; LD_LIBRARY_PATH=${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}${INSTANCE_PATH:+:${INSTANCE_PATH}/lib} ${TCMOPT} ${INSTANCE_PATH}/${FUSE_BIN_REL_PATH} ${FUSE_OPTS} $INSTANCE_MNT --lio -C ${WDIR} -log ${INSTANCE_PATH}/logs/fuse.cfg $LIO_OPTS ${LIO_CONFIG}"
    sudo -u ${RUN_AS_USER} timeout -s 9 30 bash -c "ulimit -c unlimited; ulimit -n 20480; LD_LIBRARY_PATH=${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}${INSTANCE_PATH:+:${INSTANCE_PATH}/lib} ${TCMOPT} ${INSTANCE_PATH}/${FUSE_BIN_REL_PATH} ${FUSE_OPTS} $INSTANCE_MNT --lio -C ${WDIR} -log ${INSTANCE_PATH}/cfg/lio-fuse.cfg $LIO_OPTS ${LIO_CONFIG}"
